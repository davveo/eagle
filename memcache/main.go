...
func main() {
	...
	//set key-value
	_ := mc.Set(&memcache.Item{Key: "foo", Value: []byte("my value")})
	...
	///Add a new key-value
	_ = mc.Add(&memcache.Item{Key: "foo", Value: []byte("bluegogo")})
	it, err := mc.Get("foo")
	if err != nil {
	...
	_ = mc.Replace(&memcache.Item{Key: "foo", Value: []byte("mobike")})
	it, err = mc.Get("foo")
	if err != nil {
